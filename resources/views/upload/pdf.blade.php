                ...
                <ul class="metismenu list-unstyled" id="side-menu">
                    ...
                    <li class="mm-active">
                        ...
                        <ul class="sub-menu mm-show" aria-expanded="true">
                            ...
                            <li class="mm-active"><a href="{{ route('pdfs.create') }}" class="active">Add PDF</a></li>
            ...
            <div class="page-title-box">
                <div class="container-fluid">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h4 class="page-title mb-1">Add PDF</h4>
                    ...
                    <div class="row g-3">
                            ...
                            <div class="card mb-0">
                                <div class="card-body">
                                    <h5 class="header-title mb-4">
                                        <i class="mdi mdi-file-pdf-box me-1" style="color:#e74c3c;"></i>
                                        Upload New PDF
                                    </h5>
                                    <form action="{{ route('pdfs.store') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="form-label" style="font-size:13px;">PDF Title <span class="text-danger">*</span></label>
                                            <input class="form-control" type="text" placeholder="Enter PDF title" name="title" required>
                                        </div>
                                        <div class="mb-4">
                                            <label class="form-label" style="font-size:13px;">PDF File <span class="text-danger">*</span></label>
                                            <div class="upload-zone" onclick="document.getElementById('pdf-file').click()">
                                                <i class="mdi mdi-file-pdf-box" style="font-size:32px;color:#e74c3c;"></i>
                                                <div style="font-size:13px;color:#8a9bb0;margin-top:8px;">Click to upload PDF</div>
                                                <div style="font-size:11px;color:#8a9bb0;">PDF only — max 10MB</div>
                                            </div>
                                            <input type="file" id="pdf-file" name="file" accept=".pdf" required style="display:none;"
                                                   onchange="showFileName(this)">
                                            <div id="pdf-info" style="display:none;margin-top:8px;font-size:13px;color:#019DEA;"></div>
                                        </div>
                                        <button class="btn btn-primary w-100" type="submit">
                                            <i class="mdi mdi-upload me-1"></i>Upload PDF
                                        </button>
                                    </form>
                                </div>
                            </div>
                        ...
                        <div class="col-xl-8">
                            <div class="card mb-0">
                                <div class="card-body">
                                    <h5 class="header-title mb-4">
                                        <i class="mdi mdi-file-multiple me-1" style="color:#3051d3;"></i>
                                        My PDFs
                                        <span class="badge ms-2" style="background:rgba(1,157,234,0.15);color:#019DEA;">{{ $pdfs->count() }}</span>
                                    </h5>
                                    @if($pdfs->isEmpty())
                                    <div class="text-center py-5">
                                        <i class="mdi mdi-file-pdf-box" style="font-size:48px;color:#8a9bb0;"></i>
                                        <p class="text-muted mt-3">No PDFs uploaded yet.</p>
                                    </div>
                                    @else
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="d-flex align-items-center gap-2">
                                                            <i class="mdi mdi-file-pdf-box" style="font-size:20px;color:#e74c3c;"></i>
                                                            <span style="font-weight:600;color:#e4e4e4;">{{ $pdf->title }}</span>
                                                        </div>
                                                    </td>
                                                    <td style="color:#8a9bb0;">{{ optional($pdf->user)->name ?? '—' }}</td>
                                                    <td style="color:#8a9bb0;">{{ $pdf->created_at->format('d M Y') }}</td>
                                                    <td>
                                                        <a href="{{ asset('storage/' . $pdf->file_path) }}" target="_blank" class="btn btn-outline-primary btn-sm">
                                                            <i class="mdi mdi-eye me-1"></i>Open
                                                        </a>
                                                    </td>
                                                </tr>
                                                @endforeach
...
function showFileName(input) {
    if (input.files && input.files[0]) {
        ...
        document.getElementById('pdf-info').innerHTML =
            '<i class="mdi mdi-check-circle text-success me-1"></i>' + input.files[0].name + ' (' + size + ' MB)';
    }
}
</script>
</body>
